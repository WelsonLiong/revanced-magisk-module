...
--keystore-entry-password=s3cur3p@ssw0rd --keystore-password=s3cur3p@ssw0rd --signer=alias --keystore-entry-alias=alias $patcher_args"
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		if [ "$build_mode" = apk ]; then
            ...
            local apk_output="${BUILD_DIR}/${app_name_l}-${rv_brand_f}-v${version_f}-patches-v${rv_patches_ver}-${arch_f}.apk"
